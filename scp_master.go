...
func scp_restart_services() {
	...
	cmd := exec.Command("/usr/bin/systemctl", "restart scp_orc")
	err := cmd.Run()
	if err != nil {
		checkErr(err)
	...
	cmd = exec.Command("/usr/bin/systemctl", "restart scp_back")
	err = cmd.Run()
	if err != nil {
		checkErr(err)
	...
	cmd = exec.Command("/usr/bin/systemctl", "restart scp_master")
	err = cmd.Run()
	if err != nil {
		checkErr(err)
